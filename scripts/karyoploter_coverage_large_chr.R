...
#CHM13.genome <- toGRanges("/.mounts/labs/simpsonlab/users/mmolnar/data/references/CHM13/CHM13_v2.0_chromosome_sizes_for_plots.tsv")
#CHM13.cytobands <- toGRanges("/.mounts/labs/simpsonlab/users/mmolnar/data/references/CHM13/chm13v2.0_cytobands_allchrs.txt")
#kp <- plotKaryotype(genome = CHM13.genome, cytobands = CHM13.cytobands, labels.plotter = NULL, plot.params = pp, plot.type=1)
kp <- plotKaryotype(genome=args[4], chromosomes=c("chr1", "chr2", "chr3", "chr4", "chr5", "chr6", "chr7", "chr8", "chr9", "chr10", "chr11", "chrX"), labels.plotter = NULL, plot.params = pp, plot.type=1)
kpAddChromosomeNames(kp)
#kpAddCytobandLabels(kp, col="red4")
...
legend("right", fill = c("blue", "red"), legend = c(args[5], args[6]), bty="n", ncol=1, cex=1.5)
dev.off()